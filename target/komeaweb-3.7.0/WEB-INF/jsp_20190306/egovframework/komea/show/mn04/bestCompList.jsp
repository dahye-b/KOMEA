            ...
            <div class="row">
            	...
            	<div class="section-text col-12 mb-100 bo-l">
					...
					<div class="table-responsive text-nowrap">
						 <table class="table table-bordered border-top-3 table-hover" summary="우수기업선정 게시판 입니다." >
					 		<caption class="d-none" >우수기업선정</caption>
							<thead>
								<tr>
										...
										<c:forEach var="resultList" items="${resultList}" varStatus="status">
											<tr>
												...
												<td class="text-center td-hide-md ">
													<c:choose>
													<c:when test="${resultList.etc1 eq '완료'}">
														<span class="btn btn-xs btn-primary rounded-1">${resultList.etc1}</span>
													</c:when>
													<c:when test="${resultList.etc1 eq '진행'}">
														<span class="btn btn-xs btn-success rounded-1">${resultList.etc1}</span>
													</c:when>
													<c:otherwise>
														<span class="btn btn-xs btn-secondary rounded-1">${resultList.etc1}</span>
													</c:otherwise>
													</c:choose>
												</td>
