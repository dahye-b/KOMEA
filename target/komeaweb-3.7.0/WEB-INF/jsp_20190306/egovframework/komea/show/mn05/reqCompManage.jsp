...
//진행사항 업데이트UpdateProgressStatus
function fn_ChangeStatus(obj1, obj2, obj3 ,obj4){
	document.ListForm.gubun.value = obj1;
	document.ListForm.code.value = obj2;
	document.ListForm.entrprsMberCd.value = obj3;
	document.ListForm.data.value = obj4;
	document.ListForm.action = "<c:url value='/show/UpdateSlctnYn.do'/>";
	document.ListForm.submit();
}
//선정완료
function fn_WinCmpl() {
...
<form:input type="hidden" path="data" name="data" id="data" value=""/>
														...
														<c:otherwise>
															<c:choose>
																<c:when test="${resultList.slctnYn == 'Y'}">
																	<input type="radio" id="slctnYn" value="Y" checked OnChange="javascript:fn_ChangeStatus('${resultList.gubun}','${resultList.code}','${resultList.entrprsMberCd}','Y');"/> 선정
																	<input type="radio" id="slctnYn" value="N" OnChange="javascript:fn_ChangeStatus('${resultList.gubun}','${resultList.code}','${resultList.entrprsMberCd}','N');"/> 탈락
																</c:when>
																<c:when test="${resultList.slctnYn == 'N'}">
																	<input type="radio" id="slctnYn" value="Y" OnChange="javascript:fn_ChangeStatus('${resultList.gubun}','${resultList.code}','${resultList.entrprsMberCd}','Y');"/> 선정
																	<input type="radio" id="slctnYn" value="N" checked OnChange="javascript:fn_ChangeStatus('${resultList.gubun}','${resultList.code}','${resultList.entrprsMberCd}','N');"/> 탈락
																</c:when>
																<c:otherwise>
																	<input type="radio" id="slctnYn" value="Y" OnChange="javascript:fn_ChangeStatus('${resultList.gubun}','${resultList.code}','${resultList.entrprsMberCd}','Y');"/> 선정
																	<input type="radio" id="slctnYn" value="N" OnChange="javascript:fn_ChangeStatus('${resultList.gubun}','${resultList.code}','${resultList.entrprsMberCd}','N');"/> 탈락
