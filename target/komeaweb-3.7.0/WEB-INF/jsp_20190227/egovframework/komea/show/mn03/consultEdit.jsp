...
function fn_Insert(){
	...
	*/
	if(fm.file1.value == "" && fm.file1Nm.value == ""){
		alert("사업자등록증을 첨부하세요.");
		fm.file1.focus();
		return;
	}
	
	if(fm.file2.value == "" && fm.file2Nm.value == ""){
		alert("기본제품이미지를 첨부하세요.");
		fm.file2.focus();
		return;
	}
	
	if(fm.file4.value == "" && fm.file4Nm.value == ""){
		alert("제품카탈로그를 첨부하세요.");
		fm.file4.focus();
		return;
	}
        ...
        <div class="container">
            <div class="row">
            	...
            	<div class="section-text col-12 mb-100 bo-v show">
					<jsp:include flush="true" page="/WEB-INF/jsp/egovframework/komea/form/cnsltRequestRegistEdit.jsp" />
				    <!-- 버튼 시작 { -->
				    <div class="text-right" >
				        <a class="btn btn-sm btn-primary" href="javascript:fn_Insert();" title="신청하기" >신청하기</a>
						<a class="btn btn-sm btn-secondary" href="<c:url value='/show/ReqBizList.do'/>" title="취소" >취소</a>
				    </div>
				    <!-- } 버튼 끝 -->
				</div>
            </div>
