...
//참가바이어맇스트
function buyerList(obj1){
	$.ajax({
		url : "/show/SelectBuyerInfoList.do",
		type : "POST",
		data : "cnsltexhbCd="+obj1,
		success : function(data) {
			//console.log(data);
			var resultData = data.data;
			//alert("조회성공"+resultData[0].title);
			...
			$("#totalCnt2").empty();
			if(data.totalCnt > 0 ){
				$("#totalCnt2").append(data.totalCnt);
			}else{
				$("#totalCnt2").append("0");
			}
			...
			var contents = '';
			 $("#pointTable2").empty();
			 contents += '<thead><tr><th scope="col">국가</th><th scope="col">업체명</th><th scope="col">구분</th><th scope="col">주요 서비스</th></tr></thead><tbody>';

			$.each(resultData, function(index, value){
				contents += '<tr><td class="text-center">'+value.nationNm+'</td><td class="text-center">'+value.entrpsNm+'</td><td class="text-center" >'+value.gb+'</td><td class="text-center">'+value.mainSrvc+'</td></tr>';
			});
			contents += '</tbody>';
			$("#pointTable2").html(contents);
        ...
        <div class="container">
            <div class="row">
            	...
            	<div class="section-text col-12 mb-100 bo-w">
					<jsp:include flush="true" page="/WEB-INF/jsp/egovframework/komea/form/cnsltMoveView.jsp" />
...
<div class="modal" id="Modal2">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<!-- Modal Header -->
			<div class="modal-header">
				<h3 class="modal-title">참가 바이어리스트</h3>
				<button type="button" class="close" data-dismiss="modal">&times;</button>
			</div>
			...
			<div class="modal-body">
				<h5 class="text-center">
					<span id="totalCnt2" class="text-primary"></span>건이 검색되었습니다.
				</h5>
				<div class="table-responsive text-nowrap table-wrapper-scroll-y" >
					<table class="table table-bordered border-top-2" summary="게시판 자료" id="pointTable2" >
					</table>
				</div>
			</div>
