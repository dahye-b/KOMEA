...
function detail(obj1, obj2){
	...
	document.ListForm.action = "<c:url value='/show/ReqBiddingView.do'/>";
										...
										<c:forEach var="resultList" items="${resultList}" varStatus="status">
											<tr>
												...
												<td class="td-b-hide-sm">
													<a href="javascript:detail('${resultList.code}','${resultList.gubun}');" class="text-16">
														<c:out value="${resultList.sj}" /><br/>
														<c:out value="${resultList.fromDate}" /> ~ <c:out value="${resultList.closeDate}" /> 
													</a>
												</td>
												...
												<td class="text-center td-hide-md ">
													...
													<c:when test="${resultList.biddingStatusNm eq '선정'}">
														<span class="btn btn-xs btn-primary rounded-1">${resultList.biddingStatusNm}</span>
													</c:when>
													<c:otherwise>
														<span class="btn btn-xs btn-secondary rounded-1">${resultList.biddingStatusNm}</span>
													</c:otherwise>
													</c:choose>
												</td>
												<td class="text-center td-hide-md">
													<c:if test="${resultList.biddingStatusNm eq '선정'}">
														<span class="btn btn-xs btn-info rounded-1">일정등록</span>
													</c:if>
												</td>
											</tr>
										</c:forEach>
