		...
		success : function(data) {
			if(!data.isLogin){
				if (confirm("로그인이 필요합니니다. 로그인 화면으로 이동하시겠습니까?")) {
					document.ListForm.action = "<c:url value='/komea/login.do'/>";
				...
				if(data.ssbyp == "2"){
					if(data.preReq){
						...
						document.ListForm.action = "<c:url value='/show/InfoCenter/BestCompanyRequestWrite.do'/>";
						document.ListForm.submit();
					}
				}else{
            ...
            <div class="row">
            	...
            	<div class="section-text col-12 mb-100 bo-v">
				    ...
				    <div class="bo-v-title border-top-3" >
				        <h4 class="text-bold" >제목 : ${showVO.sj}<span class="btn btn-xs btn-success rounded-1">${showVO.etc1}</span></h4>
