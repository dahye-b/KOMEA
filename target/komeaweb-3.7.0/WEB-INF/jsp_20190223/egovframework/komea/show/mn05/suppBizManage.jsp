            	...
            	<div class="section-text col-12 mb-100 bo-l">
						...
						<div class="card-block bg-light">
							...
							<div class="form-inline justify-content-end bo-search px-15 py-15" >
								<div class="form-group" >
									<form:radiobutton path="gubun" value="9" label="전체" OnClick="javascript:fn_Search('');"/>
					...
					<div class="bo-category mb-10" >
						<ul class="nav">
							<li class="nav-item">
								<c:choose>
									<c:when test="${progrsSttus == null}"><a class="nav-link active" href="javascript:fn_Search()">전체</a></c:when>
									...
									<c:when test="${fn:length(resultList) > 0}">
										<c:forEach var="resultList" items="${resultList}" varStatus="status">
											<tr>
												...
												<td class="text-center td-hide-md">
													<c:choose>
														<c:when test="${resultList.progrsSttusNm == '신청'}">신청<a OnClick="javascript:fn_Delete('${resultList.gubun}','${resultList.code}');">삭제</a></c:when>
														<c:when test="${resultList.progrsSttusNm == '탈락'}">탈락</c:when>
														<c:otherwise>
												...
												<td class="text-center text-nonowrap td-hide-md">
													<c:choose>
														<c:when test="${resultList.progrsSttus == '3'}">
															<a href="javascript:fn_Modify('${resultList.code}','${resultList.gubun}');" class="btn btn-xs btn-primary wd-60" >수정</a>
															<c:if test="${resultList.progrsSttusNm == '신청'}">
																<a href="javascript:fn_Delete('${resultList.code}','${resultList.gubun}');" class="btn btn-xs btn-warning wd-60" >삭제</a>
															</c:if>
														</c:when>
														<c:otherwise>
															<c:choose>
