...
function fn_Save(){
	...
	//개최규모
	if(fm.totalPartcptScale.value != ""){
		fm.totalPartcptScale.value = parseInt(fm.totalPartcptScale.value.replace(/,/g,""));
	}
	if(fm.totalPartcptCnt.value != ""){
		fm.totalPartcptCnt.value = parseInt(fm.totalPartcptCnt.value.replace(/,/g,""));
	}
	if(fm.totalPartcptPeople.value != ""){
		fm.totalPartcptPeople.value = parseInt(fm.totalPartcptPeople.value.replace(/,/g,""));
	}
	//한국관 참가규모
	if(fm.korPartcptScaleNum.value != ""){
		fm.korPartcptScaleNum.value = parseInt(fm.korPartcptScaleNum.value.replace(/,/g,""));
	}
	if(fm.korPartcptCompCnt.value != ""){
		fm.korPartcptCompCnt.value = parseInt(fm.korPartcptCompCnt.value.replace(/,/g,""));
	}
	if(fm.korPartcptSpceCnt.value != ""){
		fm.korPartcptSpceCnt.value = parseInt(fm.korPartcptSpceCnt.value.replace(/,/g,""));
	}
	//상담실적
	if(fm.cnsltCnt.value != ""){
		fm.cnsltCnt.value = parseInt(fm.cnsltCnt.value.replace(/,/g,""));
	}
	if(fm.cnsltMoney.value != ""){
		fm.cnsltMoney.value = parseInt(fm.cnsltMoney.value.replace(/,/g,""));
	}
	//계약(추진)액
	if(fm.contractCnt.value != ""){
		fm.contractCnt.value = parseInt(fm.contractCnt.value.replace(/,/g,""));
	}
	if(fm.contractMoney.value != ""){
		fm.contractMoney.value = parseInt(fm.contractMoney.value.replace(/,/g,""));
	}
