...
function fn_Excel(){
	document.ListForm.action = "<c:url value='/admin/compListExcel.do'/>";
	document.ListForm.submit();
}
...
//지역수정
function fn_ChangeArea(obj1, obj2) {
	$.ajax({
		url : "/admin/UpdateCompArea.do", 
		type : "POST",  
		data : "id="+obj1+"&area="+obj2,
		success : function(data) {
			var msg = data.msg;
			if(msg==="OK"){
				alert("조합원사 지역을 수정 하였습니다.");
				location.reload();
			}
		},
		error:function(request,status,er){
			...
			<table class="table table-bordered" summary="게시판관리 자료">
				...
				<thead>
					<tr>
						...
						<th scope="col" class="td-date" >지역수정</th>
					</tr>
				</thead>
				<tbody>
					<c:choose>
						<c:when test="${fn:length(resultList) > 0}">
							...
							<tr>
								...
								<td class="text-center">
									<select id="status" name="status" OnChange="javascript:fn_ChangeArea('${resultList.id}', this.value);">
										<option value="" selected>국내지역선택</option>
										<option value="SU">서울</option>
										<option value="SJ">세종</option>
										<option value="IN">인천</option>
										<option value="GN">경기도</option>
										<option value="GG">강원도</option>
										<option value="CB">층청북도</option>
										<option value="CN">충청남도</option>
										<option value="GW">대전</option>
										<option value="DE">대구</option>
										<option value="JB">전라북도</option>
										<option value="JN">전라남도</option>
										<option value="GZ">광주</option>
										<option value="KP">경상북도</option>
										<option value="KN">경상남도</option>
